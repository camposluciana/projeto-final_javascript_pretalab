...
function calcularMedia() {
    ...
    //de acordo com cada nota, analisar a situacao
   ...
   if(media === 0) {
    ...
    mensagem = `Você obteve media ${media.toFixed(1)}! Falta pouco para a média`;
   } else if(media >= 6 && media <= 7) {
    mensagem = `Você está na média com ${media.toFixed(1)}`;
   } else if(media <= 7.1 && media <= 9.9) {
    mensagem = `Notão! Sua média é ${media.toFixed(1)}`;
   } else if(media === 10) {
    mensagem = `Hoje é seu aniversário? Por que você tá de parabéns! 10 de média!`;
   }
